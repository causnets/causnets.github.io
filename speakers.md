...
### 13. Shishir Adhikari, Icahn School of Medicine at Mount Sinai

#### Learning Exposure Mapping Functions for Inferring Heterogeneous Peer Effects

In network settings, peer effects capture how peer exposures, the extent to which the unit is exposed to the treatments, actions, or behaviors of others, affect one’s outcomes. Peer exposure is explicitly defined with an exposure mapping function (EMF) that aggregates peer treatments and outputs peer exposure. However, the true function is rarely known in practice, and misspecification leads to biased estimates of causal effects. Our work focuses on moving away from the need to explicitly define an EMF and instead introduces a framework that automatically learns this function. We develop EgoNetGNN, a graph neural network (GNN), for heterogeneous peer effect estimation that automatically learns the appropriate EMF and supports complex peer exposure mechanisms that involve not only peer treatments but also local neighborhood attributes. We theoretically and empirically show that GNN models that use arbitrary EMF, or that learn peer exposure naively, struggle to account for these influence mechanisms. Our evaluation on synthetic and semi-synthetic network data shows that our method is more robust to different unknown underlying influence mechanisms than state-of-the-art baselines.
